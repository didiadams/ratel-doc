...
Ratel Security processes every financial transaction through a **multi-layer screening pipeline** in under 200ms. Each layer produces an independent verdict with a score, outcome, and set of downstream actions. These verdicts are aggregated using a **weighted severity model** to produce a final decision.
