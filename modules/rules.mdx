...
<CodeGroup>
```bash cURL
curl -X POST https://api.ratel.security/v1/rules \
...
```javascript Node.js
const response = await fetch("https://api.ratel.security/v1/rules", {
  method: "POST",
  headers: {
...
response = requests.post(
    "https://api.ratel.security/v1/rules",
    headers={
        "Authorization": "Bearer <access_token>",
        "Content-Type": "application/json"
    },
...
```bash
curl -X GET "https://api.ratel.security/v1/rules?status=ACTIVE&page=1&limit=20" \
  -H "Authorization: Bearer <access_token>"
```
...
```bash
curl -X GET https://api.ratel.security/v1/rules/rule_789xyz \
  -H "Authorization: Bearer <access_token>"
```
...
```bash
curl -X PATCH https://api.ratel.security/v1/rules/rule_789xyz \
  -H "Authorization: Bearer <access_token>" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X PATCH https://api.ratel.security/v1/rules/rule_789xyz/activate \
  -H "Authorization: Bearer <access_token>"
```
...
Temporarily disable a custom rule without deleting it.
...
```bash
curl -X PATCH https://api.ratel.security/v1/rules/rule_789xyz/pause \
  -H "Authorization: Bearer <access_token>"
```
...
```bash
curl -X DELETE https://api.ratel.security/v1/rules/rule_789xyz \
  -H "Authorization: Bearer <access_token>"
```
