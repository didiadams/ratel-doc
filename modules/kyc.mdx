...
| Mode | Description |
|------|-------------|
| `EMBEDDED` | Uses platform-managed providers (Prembly → Dojah fallback). Default for all new tenants. |
...
```json
{
  ...
  "endpoints": { "nin": "/id_verification", "bvn": "/id_verification", "liveness": "/biometric" },
...
```bash
curl -X POST https://api.ratel.security/v1/kyc/kyb/applications \
  -H "Authorization: Bearer <access_token>" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST https://api.ratel.security/v1/kyc/kyb/applications/kyb_abc123/verify-cac \
  -H "Authorization: Bearer <access_token>" \
  -H "Content-Type: application/json" \
  -d '{
    "rcNumber": "RC123456"
...
```bash
curl -X POST https://api.ratel.security/v1/kyc/applications/kyc_abc123/verify-bvn \
  -H "Authorization: Bearer <access_token>" \
  -H "Content-Type: application/json" \
  -d '{
    "bvn": "22012345678"
...
```bash
curl -X POST https://api.ratel.security/v1/kyc/applications/kyc_abc123/verify-nin \
  -H "Authorization: Bearer <access_token>" \
  -H "Content-Type: application/json" \
  -d '{
    "nin": "12345678901"
...
```bash
curl -X POST https://api.ratel.security/v1/kyc/applications/kyc_abc123/liveness-check \
  -H "Authorization: Bearer <access_token>"
```
...
```bash
curl -X PATCH https://api.ratel.security/v1/kyc/applications/kyc_abc123/reject \
  -H "Authorization: Bearer <access_token>" \
  -H "Content-Type: application/json" \
  -d '{
