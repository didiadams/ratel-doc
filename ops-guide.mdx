...
description: "How to deploy, manage, and troubleshoot Ratel Security in production"
---

This guide is for the person responsible for running and maintaining Ratel Security in production. All commands assume you are working from `/opt/ratel-security` on your VPS.
...
# 3. Rebuild the app image
docker compose build ratel-app

# 4. Restart (migrations run automatically via start.sh)
docker compose up -d ratel-app
```
...
# Full stop then start (after .env changes)
docker compose down
docker compose up -d

# Force recreate app container (picks up .env changes)
...
| Update code | `git pull && docker compose build ratel-app && docker compose up -d ratel-app` |
